...
from edl_mirage.architectures.convolution_sequential import convolution_sequential
...
class ConvLinSeq(nn.Module):
    def __init__(
        ...
        self.linear = linear_sequential(
            input_dims=[
                conv_hidden_dims[-1]
                * (input_dims[0] // 2 ** len(conv_hidden_dims))
...
def convolution_linear_sequential(
    input_dims,
    linear_hidden_dims,
    conv_hidden_dims,
    output_dim,
