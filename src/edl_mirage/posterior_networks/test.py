...
from edl_mirage.posterior_networks.metrics import (
    accuracy,
    confidence,
    brier_score,
    anomaly_detection,
...
def test(model, test_loader, ood_dataset_loaders, result_path="saved_results"):
    ...
    with torch.no_grad():
        orig_Y_all, X_duplicate_all, alpha_pred_all = compute_X_Y_alpha(
            model, test_loader
        )
        ...
        metrics["confidence_aleatoric"] = confidence(
            Y=orig_Y_all,
            alpha=alpha_pred_all,
            score_type="APR",
            uncertainty_type="aleatoric",
        )
        metrics["confidence_epistemic"] = confidence(
            Y=orig_Y_all,
            alpha=alpha_pred_all,
            score_type="APR",
        ...
        for ood_dataset_name, ood_loader in ood_dataset_loaders.items():
            ...
            metrics[f"anomaly_detection_aleatoric_{ood_dataset_name}"] = (
                anomaly_detection(
                    alpha=alpha_pred_all,
            ...
            metrics[f"anomaly_detection_epistemic_{ood_dataset_name}"] = (
                anomaly_detection(
                    alpha=alpha_pred_all,
                    ood_alpha=ood_alpha_pred_all,
                    score_type="APR",
