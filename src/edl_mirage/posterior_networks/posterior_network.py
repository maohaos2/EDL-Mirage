...
from edl_mirage.architectures.resnet_sequential import resnet18
from edl_mirage.architectures.vgg_sequential import vgg16_bn
...
class PosteriorNetwork(nn.Module):
    def __init__(
        ...
        (
            ...
            self.kernel_dim,
            self.latent_dim,
        ) = (input_dims, output_dim, hidden_dims, kernel_dim, latent_dim)
        ...
        if budget_function in __budget_functions__:
            self.N, self.budget_function = (
                __budget_functions__[budget_function](N),
                budget_function,
            )
        else:
        ...
        if architecture == "linear":
            self.sequential = linear_sequential(
            ...
            self.sequential = convolution_linear_sequential(
                input_dims=self.input_dims,
            ...
            assert len(input_dims) == 3
            self.sequential = vgg16_bn(
                output_dim=self.latent_dim, k_lipschitz=self.k_lipschitz
            )
        ...
        self.linear_classifier = linear_sequential(
            input_dims=[self.latent_dim],  # Linear classifier for sequential training
        ...
        if self.density_type == "planar_flow":
            ...
            self.density_estimation = BatchedNormalizingFlowDensity(
                c=self.output_dim,
                dim=self.latent_dim,
                flow_length=n_density,
                flow_type=self.density_type.replace("batched_", ""),
            )
            ...
            self.density_estimation = nn.ModuleList(
                [
                    MixtureDensity(
                        dim=self.latent_dim,
                        n_components=n_density,
                        mixture_type=self.density_type,
                    )
                    for c in range(self.output_dim)
                ]
            )
