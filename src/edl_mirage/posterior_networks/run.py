...
from edl_mirage.posterior_networks.posterior_network import PosteriorNetwork
from edl_mirage.posterior_networks.test import test
from edl_mirage.posterior_networks.train import train, train_sequential
from edl_mirage.utils.classification_dataset import ClassificationDataset
...
def run(
    ...
    dataset = ClassificationDataset(
        f"{directory_dataset}/{dataset_name}.csv",
    ...
    train_loader, val_loader, test_loader, N = dataset.split(
        batch_size=batch_size, split=split, num_workers=4
    )
    ...
    if training_mode == "joint":
        ...
        train_losses, val_losses, train_accuracies, val_accuracies = train_sequential(
            model,
            train_loader,
            val_loader,
            max_epochs=max_epochs,
    ...
    for ood_dataset_name in ood_dataset_names:
        dataset = ClassificationDataset(
            f"{directory_dataset}/{ood_dataset_name}.csv",
            input_dims=input_dims,
            output_dim=output_dim,
            transform_min=transform_min,
            transform_max=transform_max,
        ...
        ood_dataset_loaders[ood_dataset_name] = torch.utils.data.DataLoader(
            dataset, batch_size=2 * batch_size, num_workers=4, pin_memory=True
        )
        if unscaled_ood:
            dataset = ClassificationDataset(
                f"{directory_dataset}/{ood_dataset_name}.csv",
            ...
            ood_dataset_loaders[ood_dataset_name + "_unscaled"] = (
