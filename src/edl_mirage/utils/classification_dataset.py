...
class ClassificationDataset(Dataset):
    def __init__(
        ...
        transform_min=0.0,
        transform_max=1.0,  # Min and max scaler for input data. floats
        seed=None,
    ):  # Seed to shuffle dataset. int or None (if no shuffling)
        self.seed = seed
        ...
        X, Y = (
            df.values[: self.n_data, 1 : input_dim + 1],
            df.values[: self.n_data, input_dim + 1 : input_dim + 2],
        )
    ...
    def __getitem__(self, item):
        if len(self.input_dims) == 1:  # Vector data
            x = (
                torch.from_numpy(self.X[item]).type(torch.DoubleTensor)
                - self.transform_min
            ) / self.transform_max
        elif (
            len(self.input_dims) == 3 and self.input_dims[2] == 1
        ):  # Image data with one channel
            x = (
                torch.from_numpy(self.X[item]).type(torch.DoubleTensor)
                - self.transform_min
            ) / self.transform_max
            ...
            len(self.input_dims) == 3 and self.input_dims[2] == 3
        ):  # Image data with three channel
            input_reshaped = np.uint8(
                np.transpose(
                    self.X[item].reshape(
                        self.input_dims[2], self.input_dims[0], self.input_dims[1]
                    ),
                    (1, 2, 0),
                )
            )
            x = torch.from_numpy(
                np.array(self.transform(input_reshaped), np.int32, copy=False)
    ...
    def split(self, batch_size, split=[0.6, 0.8], num_workers=0):
        indices = list(range(self.n_data))

        # Compute class counts in training set
        class_index, class_count = np.unique(
            self.Y[: int(split[0] * self.n_data)], return_counts=True
        )
        ...
        if self.seed is None:
            n_test = 10000
            i_val = int(0.8 * (self.n_data - n_test))
            ...
            train_indices, val_indices, test_indices = (
                indices[: int(split[0] * self.n_data)],
